<script setup>
    ...
    const isNavigating = ref(false); // État pour le chargement
    const page = usePage();

    /**
     * GESTION DU CHARGEMENT VISUEL
     * Supprime l'effet de page "bloquée" au clic
     */
    router.on('start', () => { isNavigating.value = true });
    router.on('finish', () => { isNavigating.value = false });

    /**
     * GESTION DES NOTIFICATIONS (Flash messages) - Tes données restent intactes
     */
    const snackbar = ref({
        show: false,
        message: "",
...
<template>
    <v-app>
            ...
            <v-container class="fill-height d-flex align-center" fluid>
                <div class="mr-4">
                    <Link :href="route('selection')" class="d-flex text-decoration-none">
                        <v-avatar color="teal-darken-2" size="40" class="elevation-2 cursor-pointer">
                            <v-icon color="white" icon="mdi-file-send"></v-icon>
                        </v-avatar>
                    </Link>
                </div>

                <div class="hidden-sm-and-down d-md-flex align-center" style="gap: 8px">
                    <Link :href="route('selection')">
                        <v-btn :active="route().current('selection')" variant="text" prepend-icon="mdi-home" class="text-none rounded-lg">
                            Materiels
                        </v-btn>
                    </Link>

                    <Link :href="route('demandes.index')">
                        <v-btn :active="route().current('demandes.index')" variant="text" prepend-icon="mdi-clipboard-text-clock-outline" class="text-none rounded-lg">
                            Liste
                        </v-btn>
                    </Link>

                    <Link :href="route('demandes.gestion_service')">
                        <v-btn :active="route().current('demandes.gestion_service')" variant="text" prepend-icon="mdi-office-building-cog" class="text-none rounded-lg">
                            Par Service
                        </v-btn>
                    </Link>

                    <Link :href="route('demandes.historique')">
                        <v-btn :active="route().current('demandes.historique')" variant="text" prepend-icon="mdi-history" class="text-none rounded-lg">
                            Historique
                        </v-btn>
                    </Link>

                    <Link :href="route('demandes.create')">
                        <v-btn variant="flat" color="teal-darken-1" prepend-icon="mdi-send-plus" class="ml-4 text-none rounded-pill elevation-2 px-6">
                            Nouvelle Demande
                        </v-btn>
                    </Link>
                </div>
                ...
                <v-menu min-width="240px" rounded="xl" transition="slide-y-transition">
                    <template v-slot:activator="{ props }">
                        <v-btn icon v-bind="props" class="ml-2">
                            <v-avatar color="teal-lighten-4" size="38" class="border">
                    ...
                    <v-list class="pa-2" nav elevation="10">
                        ...
                        <Link :href="route('profile.edit')" class="text-decoration-none">
                            <v-list-item prepend-icon="mdi-account-circle-outline" title="Mon Profil" class="rounded-lg"></v-list-item>
                        </Link>
        ...
        <v-navigation-drawer v-model="drawer" temporary width="280">
            <v-list nav>
                <Link :href="route('selection')">
                    <v-list-item prepend-icon="mdi-arrow-left" title="Retour Materiels" class="text-teal-darken-3 font-weight-bold"></v-list-item>
                </Link>
        ...
        <v-main class="bg-teal-lighten-5">
            <header v-if="$slots.header" class="bg-white border-b-lg border-teal-darken-1 elevation-2 mb-6">
                <v-container class="py-6" fluid>
                    <h1 class="text-h4 font-weight-black text-teal-darken-4 px-4">
                        <slot name="header" />
                    </h1>
                </v-container>
            </header>
        ...
        <v-snackbar v-model="snackbar.show" :color="snackbar.color" location="top right" elevation="24" rounded="pill" :timeout="4500">
