...
// Calculer le nombre de commandes uniques (groupées par numcomande)
const nombreCommandes = computed(() => {
    if (!props.demandes?.data) return 0;
    const commandesUniques = new Set();
    props.demandes.data.forEach(d => {
        if (d.numcomande) commandesUniques.add(d.numcomande);
    });
    return commandesUniques.size;
});
...
const commandesEnAttente = computed(() => {
    ...
    const commandesUniques = new Set();
    props.demandes.data.forEach(d => {
        if (d.statut?.toLowerCase() === "en attente" && d.numcomande) {
            commandesUniques.add(d.numcomande);
        }
    });
    return commandesUniques.size;
});
        ...
        <v-container fluid class="pa-4 bg-grey-lighten-4 min-h-screen">
            ...
            <v-row class="mb-4" align="center" dense>
                <v-col cols="12" md="5">
                    <v-text-field v-model="search" prepend-inner-icon="mdi-magnify" placeholder="Rechercher..."
                        variant="solo" flat hide-details rounded="lg" bg-color="white" clearable></v-text-field>
                </v-col>
                <v-spacer></v-spacer>
            ...
            <v-card class="rounded-xl shadow-lg" border flat>
                <div style="max-height: 500px; overflow-y: auto; display: block;">
                    <v-data-table :headers="headers" :items="demandes.data" :group-by="groupBy"
                        class="demandes-table-compact" hide-default-footer density="compact">
                                ...
                                <td :colspan="columns.length" class="py-0 px-2">
                                    <div class="d-flex align-center">
                                        <v-btn :icon="isGroupOpen(item) ? 'mdi-chevron-down' : 'mdi-chevron-right'"
                                            variant="text" size="x-small" @click="toggleGroup(item)"></v-btn>

                                        <v-chip color="teal-darken-4" size="x-small" class="mr-2 font-weight-black"
                                            style="height: 18px; font-size: 0.65rem !important;">
                                            #{{ item.value }}
                                        </v-chip>
                                        ...
                                        <span class="text-caption" style="font-size: 0.7rem !important;">
                                            ...
                                            <b class="text-grey-darken-3">{{ item.items[0]?.raw?.service_beneficiaire ||
                                                item.items[0]?.service_beneficiaire }}</b>
                                        </span>

                                        <v-spacer></v-spacer>

                                        <v-chip
                                            :color="getStatusConfig(item.items[0]?.raw?.statut || item.items[0]?.statut).color"
                                            size="x-small" variant="flat" class="mr-2 font-weight-bold"
                                            style="height: 18px; font-size: 0.6rem !important;"
                                            :prepend-icon="getStatusConfig(item.items[0]?.raw?.statut || item.items[0]?.statut).icon">
                                            {{ item.items[0]?.raw?.statut || item.items[0]?.statut }}
                                        </v-chip>

                                        <v-btn icon="mdi-delete-sweep" color="red" variant="text" size="x-small"
                                            @click="openDeleteModal(item.items[0]?.raw || item.items[0])"></v-btn>
                                    </div>
                                </td>
                            </tr>
                        </template>
                            ...
                            <tr v-if="item && (item.raw || item)" style="height: auto !important;">
                                <td :colspan="headers.length" class="pa-0 border-0">
                                    <v-card variant="outlined" border
                                        class="rounded-lg my-1 bg-white ml-8 mr-2 px-2 py-1"
                                        style="border-color: #e0e0e0 !important;">
                                        <div class="d-flex align-center">
                                            <div class="flex-grow-1">
                                                <template
                                                    v-if="!(item.raw || item).est_sortie_materiel && (item.raw || item).pieces?.length > 0">
                                                    <div v-for="piece in (item.raw || item).pieces"
                                                        :key="'only-piece-' + piece.id">
                                                        <div class="d-flex align-center">
                                                            <v-avatar size="20" color="orange-lighten-4" class="mr-2">
                                                                <v-icon color="orange-darken-3"
                                                                    size="14">mdi-puzzle</v-icon>
                                                            </v-avatar>
                                                            <span
                                                                class="font-weight-black text-uppercase text-orange-darken-4"
                                                                style="font-size: 0.85rem !important;">
                                                                {{ piece.nom_piece }}
                                                            </span>
                                                            <v-chip size="x-small" variant="outlined"
                                                                color="orange-darken-2" class="ml-2 px-1"
                                                                style="height: 16px; font-size: 0.6rem !important;">
                                                                PIÈCE DÉTACHÉE
                                                            </v-chip>
                                                        </div>
                                                        <div class="text-grey ml-7 mt-1"
                                                            style="font-size: 0.65rem !important;">
                                                            Origine : {{ (item.raw || item).nom_materiel }} (S/N Mat :
                                                            {{ (item.raw || item).numero_serie || 'N/A' }})
                                                        </div>
                                                    </div>
                                                </template>
                                                ...
                                                <template v-else>
                                                    <div class="d-flex align-center">
                                                        ...
                                                        <v-chip v-if="(item.raw || item).a_des_pieces_au_total"
                                                            size="x-small" color="success" variant="flat"
                                                            class="ml-2 px-1 font-weight-bold"
                                                            style="height: 14px; font-size: 0.55rem !important;">
                                                            COMPLET
                                                        </v-chip>
                                                    </div>
                                                    <div v-if="(item.raw || item).pieces?.length > 0"
                                                        class="ml-6 mt-1 pl-2">
                                                        <div v-for="piece in (item.raw || item).pieces"
                                                            :key="'inc-piece-' + piece.id"
                                                            class="d-flex align-center mb-1">
                                                            <v-icon color="orange-darken-2" size="10"
                                                                class="mr-1">mdi-plus-circle-outline</v-icon>
                                                            <span style="font-size: 0.65rem !important;"
                                                                class="text-grey-darken-2">
                                                                {{ piece.nom_piece }} (S/N: {{ piece.numero_serie ||
                                                                    'N/A' }})
                                                            </span>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="text-right" style="min-width: 40px">
                                                <span class="text-grey mr-1"
                                                    style="font-size: 0.6rem !important;">QTÉ:</span>
                                                <span class="font-weight-black" style="font-size: 0.8rem !important;">
                                                    {{ (item.raw || item).est_sortie_materiel ? (item.raw ||
                                                        item).nbredemande : (item.raw ||
                                                            item).pieces.length }}
                                                </span>
                                            </div>
                                        </div>
                ...
                <div class="pa-2 d-flex align-center justify-space-between bg-white rounded-b-xl">
                    ...
                    <v-pagination v-model="demandes.current_page" :length="demandes.last_page" :total-visible="3"
            ...
            <v-dialog v-model="deleteDialog" max-width="400">
                <v-card class="rounded-xl pa-2">
                    <v-card-title class="d-flex align-center">
                        <v-avatar color="red-lighten-5" class="mr-3"><v-icon icon="mdi-delete-alert"
                                color="red"></v-icon></v-avatar>
                        Confirmation
                    </v-card-title>
                    <v-card-text>Voulez-vous annuler la commande <b>#{{ selectedDemande?.numcomande }}</b>
                        ?</v-card-text>
