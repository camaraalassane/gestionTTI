...
<template>
    ...
    <div class="print-page">
        <div class="no-print d-flex justify-center pa-6 bg-grey-lighten-3">
            <v-btn variant="tonal" color="grey-darken-3" prepend-icon="mdi-arrow-left" @click="retourner" class="mr-4">Retour</v-btn>
            <v-btn color="black" prepend-icon="mdi-printer" @click="lancerImpression" elevation="4">Imprimer le Bon de livraison</v-btn>
        </div>
        ...
        <div class="document">
            ...
            <table class="main-table">
                ...
                <tbody>
                    <tr v-for="(item, index) in demandesDuBeneficiaire" :key="item.id">
                        <td class="text-center font-weight-bold">
                            {{ index + 1 }}
                        </td>

                        <td class="pa-2">
                            <template v-if="item.est_uniquement_piece">
                                <div v-for="piece in item.pieces" :key="piece.id" class="font-weight-bold text-uppercase">
                                    {{ piece.nom_piece }}
                                </div>
                                <div class="text-caption italic ml-2" style="color: #666">
                                    issu de {{ item.nom_materiel }}
                                    <span>(S/N Mat: {{ item.numero_serie || "N/A" }})</span>
                                </div>
                            </template>

                            <template v-else>
                                <div class="font-weight-bold text-uppercase">
        {{ item.nom_materiel || 'MATÉRIEL' }}
    </div>

                                <div v-if="item.pieces && item.pieces.length > 0">
                                    <div v-for="piece in item.pieces" :key="piece.id" class="text-caption italic ml-2">
                                        + {{ piece.nom_piece }} (S/N: {{ piece.numero_serie }})
                                    </div>
                                </div>

                                <div v-else-if="item.a_des_pieces_au_total" class="text-caption italic ml-2" style="color: #d32f2f">
                                    (Matériel seul)
                                </div>

                                <div v-else class="text-caption italic ml-2" style="color: #444">
                                    (Matériel)
                                </div>
                            </template>
                        </td>
                        ...
                        <td class="text-center font-mono font-weight-bold">
                            <template v-if="item.est_uniquement_piece && item.pieces?.length">
                                <div v-for="piece in item.pieces" :key="piece.id">
                                    {{ piece.numero_serie || "—" }}
                                </div>
                            </template>
                            <template v-else>
                    ...
                    <tr v-for="n in Math.max(0, 8 - demandesDuBeneficiaire.length)" :key="'empty-' + n" class="empty-row">
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                ...
                <tfoot>
                    <tr>
                        <td colspan="3" class="text-right pa-2 font-weight-black">
                            TOTAL ARTICLES
                        </td>
                        <td class="text-center font-weight-black bg-light text-h6 border-double">
                            {{ totalArticles }}
                        </td>
                    </tr>
                </tfoot>
            ...
            <div class="signature-grid mt-8">
                <div class="sig-card">
                    <div class="sig-header">Officier Materiel</div>
                    <div class="sig-body">
                ...
                <div class="sig-card">
                    <div class="sig-header">SOUS DIRECTEUR ST</div>
                    <div class="sig-body">
                        <div class="sig-date">Date: ____/____/20__</div>
                        <div class="sig-space">Signature</div>
                    </div>
