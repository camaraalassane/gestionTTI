    ...
    router.post(
        ...
        },
        {
            onSuccess: () => {
                ...
                // ✅ Navigation Inertia propre
                router.get(route('materiel.indexmat'));
            },
            onError: (errors) => {
                isProcessing.value = false;
        ...
        <v-main class="d-flex align-center justify-center">
            ...
            <v-card class="profile-fixed-top ma-4 ma-md-8 pa-2 pr-4 rounded-pill elevation-2" color="white" style="
                ...
                <div class="d-flex align-center">
                    ...
                    <div class="text-left mr-4 d-none d-sm-block">
                        ...
                        <div class="text-caption text-teal-darken-1 font-weight-bold text-uppercase"
            ...
            <v-container class="py-12">
                <div class="text-center mb-12 mt-10 mt-md-0">
                    ...
                    <h1 class="text-h3 text-md-h2 font-weight-black text-teal-darken-4 mb-2">
                        Gestion <span class="text-teal-darken-2"> du materiel de la DTTIA</span>
                    </h1>
                ...
                <v-row justify="center" class="px-4">
                    <!-- ✅ Carte Matériel corrigée -->
                    <v-col cols="12" md="5" lg="4">
                        <v-card @click="handleMaterialAccess" elevation="8"
                            class="module-card pa-8 rounded-xl text-center h-100 d-flex flex-column border-t-lg border-teal-darken-2"
                            color="white">
                            <div class="flex-grow-1">
                                <v-avatar :color="hasMaterialCode ? 'teal-lighten-5' : 'red-lighten-5'" size="80"
                                    class="mb-6">
                                    <v-icon :icon="hasMaterialCode ? 'mdi-package-variant' : 'mdi-lock-alert'" size="40"
                            ...
                            <v-btn :color="hasMaterialCode ? 'teal-darken-2' : 'red-darken-3'" rounded="pill"
                                class="action-btn px-8 font-weight-bold align-self-center text-none">
                                Entrer
                            </v-btn>
                        </v-card>
                    </v-col>

                    <!-- ✅ Carte Demandes corrigée -->
                    <v-col cols="12" md="5" lg="4">
                        <v-card @click="navigateTo('demandes.index')" elevation="8"
                            class="module-card pa-8 rounded-xl text-center h-100 d-flex flex-column border-t-lg border-teal-darken-2"
                            color="white">
                            <div class="flex-grow-1">
                                <v-avatar color="teal-lighten-5" size="80" class="mb-6">
                                    <v-icon icon="mdi-file-document-edit-outline" size="40"
                                        color="teal-darken-2"></v-icon>
                                </v-avatar>
                                <h2 class="text-h5 font-weight-black mb-4 text-uppercase text-teal-darken-4">
                                    Gestion Demandes
                                </h2>
                            ...
                            <v-btn color="teal-darken-2" rounded="pill"
                                class="action-btn px-8 font-weight-bold align-self-center text-none">
                                Entrer
                            </v-btn>
                        </v-card>
            ...
            <v-dialog v-model="showCodeModal" max-width="400" persistent>
                <v-card class="rounded-xl pa-6 text-center border-t-lg border-teal-darken-2" color="white">
                    ...
                    <v-card-title class="text-h5 font-weight-black text-teal-darken-4">Clé d'accès
                        requise</v-card-title>
                    <v-card-text>
                        <p class="text-grey-darken-1 mb-6">Saisissez votre code matériel pour déverrouiller ce module.
                        </p>
                        <v-text-field v-model="inputCode" label="Code Matériel" variant="outlined" color="teal-darken-2"
                            :error-messages="errorMessage" :loading="isProcessing" type="password"
                            @keyup.enter="verifyAndEnter" />
                    </v-card-text>
                    <v-card-actions class="justify-center">
                        <v-btn variant="text" color="grey" @click="showCodeModal = false">Annuler</v-btn>
                        <v-btn color="teal-darken-2" class="px-8 font-weight-bold"
                            @click="verifyAndEnter">Valider</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
            ...
            <v-dialog v-model="showDeniedModal" max-width="450">
                <v-card class="rounded-xl pa-6 text-center border-t-lg border-red-darken-2" color="white">
                    ...
                    <v-card-title class="text-h5 font-weight-black text-red-darken-4 justify-center">Accès
                        Interdit</v-card-title>
                    <v-card-text class="text-grey-darken-1 text-body-1">
                        Désolé <strong>{{ userName }}</strong>, vous ne disposez pas d'un code d'accès
                        matériel.<br /><br />
                        Veuillez contacter l'administrateur système.
                    </v-card-text>
                    <v-card-actions class="justify-center mt-4">
                        <v-btn color="red-darken-2" variant="flat" rounded="pill" class="px-8 font-weight-bold"
                            @click="showDeniedModal = false">Compris</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
        </v-main>
