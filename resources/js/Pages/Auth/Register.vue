    ...
    <v-app>
        <v-container fluid class="pa-0 fill-height">
            <v-row no-gutters class="fill-height">
                <!-- Colonne gauche avec logo DTTIA (fixe) - COULEUR TEAL -->
                ...
                <v-col cols="12" md="6" class="bg-grey-lighten-4 d-flex align-center justify-center" style="height: 100vh; position: sticky; top: 0;">
                    <v-card elevation="0" rounded="xl" class="pa-6 pa-md-10 w-100 bg-white" style="max-width: 450px; max-height: 90vh; overflow-y: auto; border: 1px solid rgba(0, 0, 0, 0.08);">
                        <div class="text-center mb-6">
                            ...
                            <h2 class="text-h4 font-weight-bold mb-2 text-teal-darken-4">Inscription</h2>
                            <p class="text-grey mb-8">Rejoignez la plateforme d'archivage numérique</p>
                        </div>

                        <v-form @submit.prevent="submit">
                            <v-text-field v-model="form.name" label="Nom complet" variant="outlined" :error-messages="form.errors.name" prepend-inner-icon="mdi-account-outline" color="teal-darken-2" class="mb-2" />

                            <v-text-field v-model="form.email" label="Email" type="email" variant="outlined" :error-messages="form.errors.email" prepend-inner-icon="mdi-email-outline" color="teal-darken-2" class="mb-2" />

                            <v-text-field v-model="form.password" label="Mot de passe" :type="showPassword ? 'text' : 'password'" variant="outlined" :error-messages="form.errors.password" prepend-inner-icon="mdi-lock-outline" :append-inner-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'" color="teal-darken-2" class="mb-2" @click:append-inner="showPassword = !showPassword" />

                            <v-text-field v-model="form.password_confirmation" label="Confirmer le mot de passe" :type="showConfirmPassword ? 'text' : 'password'" variant="outlined" :error-messages="form.errors.password_confirmation" prepend-inner-icon="mdi-lock-check-outline" :append-inner-icon="showConfirmPassword ? 'mdi-eye-off' : 'mdi-eye'" color="teal-darken-2" class="mb-6" @click:append-inner="showConfirmPassword = !showConfirmPassword" />
                            ...
                            <div class="text-center mt-6">
                                ...
                                <Link :href="route('login')" class="text-teal-darken-2 text-decoration-none font-weight-bold ms-1">
                                    Connectez-vous
                                </Link>
                            </div>
                        </v-form>
